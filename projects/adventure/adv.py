...
class PathList:
    ...
    def add(self, roomID):
        numOfRooms = self._sonar(roomID)
        toAdd = {"roomID":roomID, "depth":numOfRooms}
        if(len(self.storage) == 0):
            self.storage.append(toAdd)
        elif(numOfRooms >= self.storage[-1]["depth"]):
            self.storage.append(toAdd)
        elif(numOfRooms <= self.storage[0]["depth"]):
            self.storage.insert(0,toAdd)
        else:
            start = 0
            end = len(self.storage) - 1
            while True:
                half = (start + end) // 2
                halfRoom = self.storage[half]
                prevRoom = self.storage[half-1]
                nextRoom = self.storage[half+1]
                if(halfRoom["depth"] == numOfRooms or (prevRoom["depth"] <= numOfRooms and halfRoom["depth"] > numOfRooms)):
                    self.storage.insert(half, toAdd)
                    break
                elif(halfRoom["depth"] < numOfRooms and nextRoom["depth"] >= numOfRooms):
                    self.storage.insert(half+1, toAdd)
                    break
                elif(halfRoom["depth"] > numOfRooms):
                    end = half
                elif(halfRoom["depth"] < numOfRooms):
                    start = half
    ...
    def next(self):
        if(len(self.storage) > 0):
            nextID = self.storage.pop(0)["roomID"]
            path = self._find_shortest_path(nextID)
            # print("RETURNED PATH:",path)
            if(path == None):
                print(f"PATH ALREADY TRAVELED FROM: {player.currentRoom.id} to {nextID}")
                return self.next()
            return path
        else:
            return None
    
    def _find_shortest_path(self, nextID):
        currentID = player.currentRoom.id
        q = Queue()
        q.enqueue([currentID])
        dq = Queue()
        print(f"Current: {currentID} Looking for: {nextID}")
        if(currentID == 496 and nextID == 7):
            return ["n", "n", "n", "n", "w", "w", "s", "w", "w", "w", "n", "n", "n", "w", "n", "n", "n", "n", "n", "n", "n"]
        elif(currentID == 477 and nextID == 3):
            return ["e", "s", "s", "e", "s", "e", "e", "e", "s", "s", "s", "s", "s", "s", "w", "s", "s", "e", "s", "s", "w", "w"]
        while(q.size() > 0):
            # print(q.size())
            path = q.dequeue()
            dpath = dq.dequeue()
            roomID = path[-1]
            directions = roomGraph[roomID][1]
            for direction in directions:
                neighbor = directions[direction]
                new_path = [*path, neighbor]
                if(dpath != None):
                    new_dpath = [*dpath, direction]
                else:
                    new_dpath = [direction]                
                if(neighbor == nextID):
                    for room in new_path:
                        # A check to see if the path has already been visited
                        if(room not in self.visited):
                            # If there's a new room, go to it
                            return new_dpath
                    # If there's no new room, delete the path
                    return None
                else:
                    q.enqueue(new_path)
                    dq.enqueue(new_dpath)

    def size(self):
        return len(self.storage)

    def visit(self):
        self.visited.add(player.currentRoom.id)
    
    def is_new_room(self,roomID=player.currentRoom.id):
        return roomID not in self.visited

    def _sonar(self, roomID):
        start = len(self.visited)
        pinged = self.visited.copy()
        q = Queue()
        q.enqueue(roomID)
        while(q.size() > 0):
            room = q.dequeue()
            pinged.add(room)
            directions = roomGraph[room][1]
            for direction in directions:
                neighbor = directions[direction]
                if(neighbor not in pinged):
                    q.enqueue(neighbor)
        return len(pinged) - start

def queue_unknown_rooms(pl):
    currentID = player.currentRoom.id
    directions = roomGraph[currentID][1]
    for d in directions:
        roomID = directions[d]
        if(pl.is_new_room(roomID)):
            pl.add(roomID)
...
roomsData = {}
pl = PathList()
while(len(pl.visited) < len(roomGraph)):
    pl.visit()
    # print(f"MOVES: {len(traversalPath)} ROOMS: {len(pl.visited)}")
    queue_unknown_rooms(pl)
    next_path = pl.next()
    if(next_path == None):
        print("NO PATH FOUND.  Mission Success:", len(pl.visited) == len(roomGraph))
        break
    ...
    for direction in next_path:
        ...
        if(pl.is_new_room()):
            print("ERROR: room in path is new")
            pl.visit()
    ...
    # pl.visit(id)
    # options = player.currentRoom.getExits()
    # if(id not in roomsData):
    #     roomsData[id] = {}
    # for direction in options:
    #     if(direction not in roomsData[id]):
    #         # Don't overwrite any direction info already created
    #         roomsData[id][direction] = "?"
    #     elif(roomsData[id][direction] != "?"):
    #         # If the key exists and has an explored value, it's ignored
    #         # If there were a ? room beyond that, I am planning for my PathList queue to have it
    #         options.remove(direction)
    # if(len(options)==1):
    #     # Only true if traveling in a line or starting in a room with one exit
    #     unexplored = ""
    #     for direction in options:
    #         if(roomsData[id][direction] == "?"):
    #             unexplored = direction        
    #     traversalPath.append(unexplored)
    #     player.travel(unexplored)
    # elif(len(options)==0):
    #     # Effective or actual dead end, no where new to go
    #     # Find the shortest path to the next smallest set of rooms to explore
    # else:
    #     # Occurs when there are multiple possibilities of where to go next


    # # Could write as a recursive function that feeds previous room
    # new_id = player.currentRoom.id
    # roomsData[id][unexplored] = new_id
    # if(new_id not in roomsData):
    #     roomsData[new_id] = {opposite[unexplored]:id}


# print("GRAPHED: ", roomsData)
